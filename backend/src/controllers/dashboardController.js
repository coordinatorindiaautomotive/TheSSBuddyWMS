...
async function getStats(req, res) {
  try {
    ...
    // 2. Today's Specific Snapshot Metrics (with Party vs Retail Outlet Bifurcation)
    ...
    const todaySnapshot = {
      ...
      bifurcation: {
        parties: {
          tickets: todayPT?.party_count || 0,
          qty: todayPT?.party_qty || 0,
          pending: todayPT?.party_pending || 0,
          dispatched: todayPT?.party_dispatched || 0
        },
        retailOutlets: {
          tickets: todayPT?.retail_count || 0,
          qty: todayPT?.retail_qty || 0,
          pending: todayPT?.retail_pending || 0,
          dispatched: todayPT?.retail_dispatched || 0
        }
      },
