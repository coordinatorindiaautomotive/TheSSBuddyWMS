                ...
                >
                  <div className="space-y-0.5 truncate">
                    ...
                    <div className="text-[10px] text-slate-500 flex items-center gap-2">
                      <span className="font-mono text-[#004c8f] font-bold">{p.party_code}</span>
...
export default function ReturnEntry() {
  ...
  // 3. Return Date * (by default current date)
  ...
  // 6. DMS Status (Dropdown based, not chips: 0 = NO / Pending DMS, 1 = YES / DMS Received)
  ...
  // Multi-part table rows (Part Description / Name removed as requested)
  ...
  const handleSubmit = async (e) => {
    ...
    if (invalidItems) {
      toast.show('Please fill valid Part Number/Code and positive Qty for all rows.', 'warning');
      return;
    }
  ...
  return (
    <div className="space-y-4 pb-12">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {/* Section 1: 7-Field Return Header Details */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-5 space-y-4">
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-7 pr-2.5 py-1 text-xs font-mono font-bold text-[#003366] border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#003366] focus:outline-hidden w-44"
                />
              </div>
              <button
                type="button"
                onClick={fetchNextReturnNo}
                className="text-[10px] font-bold text-[#004C8F] bg-blue-50 border border-blue-200 hover:bg-blue-100 px-2 py-1 rounded-lg flex items-center gap-1 cursor-pointer transition-colors"
          ...
          {/* 7 Ordered Header Fields Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* 1. Invoice Bill No * (Against which return is made) */}
            <div>
              <label className="block text-[11px] font-bold text-slate-700 uppercase mb-1">
                1. Invoice Bill No <span className="text-red-500">*</span>
              </label>
              <div className="relative">
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-700 uppercase mb-1">
                2. Invoice Bill Date <span className="text-red-500">*</span>
              </label>
              <div className="relative">
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-700 uppercase mb-1">
                3. Return Date <span className="text-red-500">*</span>
              </label>
              <div className="relative">
                <Calendar className="w-4 h-4 text-slate-400 absolute left-3 top-2.5" />
                <input
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-700 uppercase mb-1">
                5. Return Reason / Remark <span className="text-red-500">*</span>
              </label>
              <select
                value={remarkId}
                onChange={(e) => handleRemarkChange(e.target.value)}
                className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-800 focus:ring-2 focus:ring-[#003366] focus:outline-hidden"
              >
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-700 uppercase mb-1">
                6. DMS Status <span className="text-red-500">*</span>
              </label>
              <select
                value={isDmsReceived}
                onChange={(e) => setIsDmsReceived(parseInt(e.target.value, 10))}
                className={`w-full px-3 py-2 border rounded-xl text-xs font-bold focus:ring-2 focus:ring-[#003366] focus:outline-hidden ${
                  isDmsReceived === 1
                    ? 'bg-emerald-50 border-emerald-300 text-emerald-800'
                    : 'bg-amber-50 border-amber-300 text-amber-800'
                }`}
              >
                <option value={0}>NO (Pending DMS)</option>
            ...
            <div className="sm:col-span-2">
              <label className="block text-[11px] font-bold text-slate-700 uppercase mb-1">
                7. DMS / STR Number <span className="text-slate-400 font-normal">(Optional)</span>
              </label>
              <input
                type="text"
                ...
                className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs font-mono font-bold text-slate-800 focus:ring-2 focus:ring-[#003366] focus:outline-hidden"
              />
            </div>
          </div>
        </div>

        {/* Section 2: Multi-Part Return Items Table (NO Part Description column) */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-5 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div className="flex items-center gap-2">
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead className="bg-[#003366] text-white font-extrabold uppercase tracking-wider text-[11px]">
                <tr>
                  ...
                  <th className="p-3 min-w-[220px]">Part Number / Code *</th>
                  ...
                  <tr key={row.id || idx} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="p-2.5">
                      <input
                        type="text"
                        required
                        placeholder="e.g. PRT-99201 / Brake Shoe"
                        value={row.part_no}
                        onChange={(e) => handleItemChange(idx, 'part_no', e.target.value)}
                        className="w-full px-3 py-1.5 border border-slate-200 rounded-lg font-mono font-bold text-xs text-[#003366] focus:border-[#003366] focus:outline-hidden"
                      />
                    </td>
                    <td className="p-2.5 text-right">
                      <input
                        ...
                        className="w-full text-right px-3 py-1.5 border border-slate-200 rounded-lg font-bold text-xs text-slate-900 focus:border-[#003366] focus:outline-hidden"
                      />
                    </td>
                    <td className="p-2.5 text-right">
                      <input
                        ...
                        className="w-full text-right px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-semibold text-slate-800 focus:border-[#003366] focus:outline-hidden"
                      />
                    </td>
              ...
              <tfoot className="bg-slate-50 border-t border-slate-200 font-bold text-xs text-slate-800">
                <tr>
                  <td colSpan={2} className="p-3 text-right uppercase tracking-wider text-[11px] text-slate-600">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 space-y-2">
          <label className="block text-[11px] font-bold text-slate-600 uppercase">
            Internal Inspection / Warehouse Remarks
          </label>
          <textarea
            rows={2}
