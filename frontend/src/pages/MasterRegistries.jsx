...
export default function MasterRegistries() {
  ...
  const PaginationFooter = ({ totalItems, label = 'records' }) => {
    ...
    const cappedTotal = Math.min(totalItems, 100);
    ...
    return (
      <div className="px-6 py-3.5 bg-slate-50 border-t border-slate-200 flex flex-wrap items-center justify-between gap-2 text-xs">
        <div className="text-slate-500 font-medium text-xs">
          Showing <strong className="text-slate-900 font-bold">{cappedTotal > 0 ? start + 1 : 0}</strong> to <strong className="text-slate-900 font-bold">{end}</strong> of <strong className="text-slate-900 font-bold">{cappedTotal}</strong> {label}
          {totalItems > 100 && (
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3.5 py-1.5 rounded-lg bg-white border border-slate-200 text-slate-700 font-bold hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed shadow-xs cursor-pointer transition-all"
          >
            Previous
          </button>
